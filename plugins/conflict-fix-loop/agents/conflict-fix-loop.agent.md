...
- `stack-publish --state <path>`: require a resolved cascade, force-push every member of the stack, and then prove every member's remote branch landed on exactly the commit the cascade intended. It verifies every member from the remote whatever `gh stack push` returns, because that push is not atomic and a partial one exits non-zero; a member still on its pre-cascade commit and a member another actor moved are named as distinct causes. A member that `gh stack push` skips because it is already merged or queued is named as its own group so every member is accounted for. When a member did not land it keeps the throwaway workspace instead of removing it, names the landed, not-landed, and skipped members and where the workspace is, and re-running `stack-publish` retries the members that did not land. When every member landed but `gh stack push` still exited non-zero, it publishes and carries the reported detail so a clean publish is distinguishable from one the tool complained about.
