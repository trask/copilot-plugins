...
A cascade must check out and move every branch in the stack in turn, but git refuses to check one branch out in two worktrees of a repository, and the App routinely holds those branches in other worktrees. So the cascade runs in a throwaway clone with its own refs, created and removed by the helper. The helper freezes every member's remote head, computes each child boundary from `baseRefOid` and the original graph before moving a branch, and refuses missing, rewritten, or ambiguous lineage. It rebases locally and pushes nothing, so nothing on the remote moves until `stack-publish`, and a discarded clone or an abort leaves every remote branch untouched.
