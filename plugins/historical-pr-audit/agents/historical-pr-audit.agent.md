...
2. Add `--new-invocation` to the first `preflight` in a user invocation. Keep the returned `invocation_run` token. Add `--invocation-run <token>` to every later `preflight` in the same user invocation. Do not start a second invocation after `max_iterations_reached`.
3. For a `resume` or `continue` with no target, run `status --current --repo-root <workspace>` first and report what it finds. Do not fall back to another pull request.
4. Run `preflight` once per iteration.
