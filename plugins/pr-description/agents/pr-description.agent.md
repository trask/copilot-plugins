...
Use an explicit target and the same state path throughout one Pipeline run. A later sweep may start a fresh recommendation after the previous task completed when the head changed, or when only the title or body changed at the same head. Target, model, mutation policy, run identity, and iteration cap must remain unchanged. Exact semantic snapshots reuse the completed result without another task. The state retains prior tasks as audit history. Active, interrupted, failed, or foreign invocations cannot be adopted.
...
- `agent-task` is the standalone path and `pipeline` is the direct Pipeline path.
- Every standalone `agent-task` call requires fresh invocation-local state. Pipeline may reuse a completed result only for the exact same semantic snapshot. A changed head, or changed title/body at the same head, starts a fresh task in a later authorized sweep. The PR-level index is audit/status data only and never blocks, resumes, or seeds a later run.
- Retired commands and options for resume, prepared-result application, prior-result import, taskless-owner archival, and executable legacy reports are unsupported. Historical state remains read-only audit evidence and cannot seed a new recommendation. Start a fresh current-policy invocation.
...
- The coordinator discovers `cloud_task.py` from the separately installed `agent-tasks-runtime@trask-plugins` skill, verifies Runtime 1.0.22 by pinned SHA-256 before dispatch, and requires policy `marketplace-agent-report-recommendation-worker@1`. Authentication stays in local `gh api`.
...
- If the source changes after a hosted task starts, the helper preserves that stale task and result, marks the candidate superseded, records `head_changed`, and performs no mutation. A Pipeline run may advance only when its existing iteration allowance has another sweep; otherwise the stage is incomplete.
...
- The coordinator still verifies the recommendation candidate itself. A later Runtime release should expose the same recommendation verification as a shared helper so this plugin can drop that local duplication without changing its policy or result contract.
