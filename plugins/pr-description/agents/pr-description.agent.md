...
- Use the helper to run preflight, to store a proposal, to apply it, to validate, and to report status. Preserve validated helper state on normal completion so an orchestrator can read its outcome. Use helper `cleanup` only to abandon state that no later status reader needs. Do not rebuild those state changes with commands of your own.
...
- `propose --state <path> --expected-run-id <run_id> --title <literal-title> --body-file <path>`: store a proposal bound to this run's exact pinned head, title, and body; remove one leading UTF-8 BOM and change CRLF or CR line endings to LF; increment its durable proposal counter; and return `proposal_token`, the `body_newline` convention it will send, and whether it normalized the body file
...
Never choose a `--state` path yourself. Let `preflight` decide where the state lives, and use the exact `state` it returns for every later command. An orchestrator reads this stage's result from the path `preflight` picked, so a path you invented is one it never reads: the run looks like it did nothing while you did the work correctly. If `preflight` refuses an argument you were told to pass, stop and report the refusal exactly as the helper worded it. Never work around it by changing the command.

1. Run `preflight` once for the requested target. Pass a supplied PR URL, bare PR number, or `owner/repo#number` exactly. Omitting the target uses the branch this worktree has checked out, which works only while it is attached to one. A pipeline runs this stage from a worktree detached at the pull request head, and a detached worktree names no branch to look up, so pass the pull request explicitly whenever you have it. An orchestrator that runs this stage as part of a larger loop may also tell you its own position, as `--pipeline-run <token> --pipeline-iteration <number> --pipeline-max-iterations <number>` or as a `pipeline-run: <token> pipeline-iteration: <number> pipeline-max-iterations: <number>` line beside the target. Pass all three to `preflight` exactly as given when it names them. This stage runs once and keeps no budget, so they change nothing here and `preflight` ignores them. Never invent, edit, or carry over a value, and never change how you run because of one.
2. After preflight succeeds, call `rename_session` exactly once with `PR Description: <number> - <title>`, built from the returned `pr.number` and `pr.title`, when the runtime exposes that tool. If the tool is unavailable, continue without renaming and do not report its absence as retrospective friction. Never use an interim name and never rename again during this run.
...
4. Read the authoritative pull request diff with `gh pr diff <pr.url> --repo <pr.repo_name>`. If the command output is too large for one tool read and the tool saves it to a file, keep the returned path, read the authoritative diff from that file, and delete the file before the terminal response. The preflight `head_sha` is the immutable head for this evaluation and for any proposal. A later `validate` or `apply` verifies that the live head still matches it.
5. In your first response after you gather the diff, present the current title and description first, including an empty description, following "Displaying Title And Description".
6. Immediately after that exact text, give your evaluation of its clarity, its concision, how well it matches the diff, and how well it covers the pull request's actual final scope.
...
1. Write the proposed body exactly as UTF-8 to a body file outside the repository, next to the helper's external state file. Do not use a file in the repository. The helper removes one leading UTF-8 BOM and turns CRLF and CR into LF, so the applied body has no transport marker and always uses LF even when the pinned body uses CRLF. Never read the helper's source to choose an encoding or line ending.
