---
name: PR Description
description: "Explicit invocation only: never select automatically; use the managed GitHub Agent Tasks worker to review and update one pull request title and description."
...
- The helper discovers the managed `cloud_task.py` through the installed copilot-config manifest. It requires policy `marketplace-agent-worker@1`. Authentication stays in local `gh api`.
