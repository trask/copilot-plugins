...
- The coordinator discovers `cloud_task.py` from the separately installed `agent-tasks-runtime@trask-plugins` skill, verifies Runtime 1.0.24 by pinned SHA-256 before dispatch, and requires policy `marketplace-agent-report-recommendation-worker@1`. Authentication stays in local `gh api`.
