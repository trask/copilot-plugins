...
- If the source changes after a hosted task starts, the helper preserves that stale task and result, records `head_changed`, and performs no mutation. A Pipeline run may advance only when its existing iteration allowance has another sweep; otherwise the stage is incomplete.
