...
For merge and rebase, the managed worker writes the resolved code history immediately before one minimal semantic payload commit on the authoritative task artifact branch. The worker supplies only the summary, positional commit annotations, and explicit `command`/`result` validation evidence. The helper adds `github.copilot.agent-task-conflict-semantic-output` version 2 identity, requires exact live task, session, base, model, and branch agreement, then derives the code tip only from the semantic commit's sole parent. Native stacks continue to use independently assigned request-scoped member refs because one artifact parent cannot identify multiple member tips. The helper derives every SHA, ordered commit mapping, patch digest, unchanged-path digest, report, `github.copilot.agent-task-conflict-receipt` version 2, and `github.copilot.agent-task-conflict-result` version 3 envelope from Git and the frozen request. The coordinator rejects missing or malformed semantic output, credentials, malformed refs, artifact/code overlap, incomplete validation, stale targets, reversed merge parents, undeclared merge paths, rebase mapping drift, changed unaffected patches, missing or extra stack members, bad ordering, stale leases, and changed outside dependents. Policy versions 1 through 3 remain legacy evidence only; their artifacts are rejected rather than repaired, normalized, or promoted into a current invocation.
