...
description: "Resolve merge conflicts on a pull request and push the resolution. Run only when the user selects PR Conflict Resolver or another user-selected maintenance agent explicitly dispatches it for a reported conflict; never select it from general task context."
...
Run only after the user explicitly invokes this agent by name or its documented command, or after another user-selected maintenance agent explicitly dispatches it for a reported conflict. Never select or start this agent from general task context.
...
- `stack-format --state <path> (--format-command [--] <executable> [args...] | --no-format)`: complete the formatting checkpoint for the current stack member. Put `--state` before `--format-command`, which is the last helper option because every later argument belongs to the formatter. An optional first `--` separates the formatter command. Choose the repository's documented formatter command, such as the affected Spotless tasks, or pass `--no-format` only when the repository has no formatting step. The helper passes option-like arguments and later separators through unchanged, including `cargo fmt --manifest-path <path> -- --check`. The helper runs the command in the throwaway workspace, accepts changes only to files already changed by that member, stages and commits those changes on that member's branch, records the final tip, and then advances to the next member. A formatter failure, an unexpected dirty worktree, or a change outside the current member leaves the checkpoint pending and blocks descendants and publication. Re-running the command after an interrupted helper call reuses a recorded checkpoint instead of running the formatter twice. A successful command with no diff is valid.
