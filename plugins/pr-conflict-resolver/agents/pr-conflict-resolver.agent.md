...
- `stack-format --state <path> (--format-command <executable> [args...] | --no-format)`: complete the formatting checkpoint for the current stack member. Choose the repository's documented formatter command, such as the affected Spotless tasks, or pass `--no-format` only when the repository has no formatting step. The helper runs the command in the throwaway workspace, accepts changes only to files already changed by that member, stages and commits those changes on that member's branch, records the final tip, and then advances to the next member. A formatter failure, an unexpected dirty worktree, or a change outside the current member leaves the checkpoint pending and blocks descendants and publication. Re-running the command after an interrupted helper call reuses a recorded checkpoint instead of running the formatter twice. A successful command with no diff is valid.
