...
The helper records `preparing` ownership at the explicit state path before conflict preflight. A preflight error records failed or interrupted ownership with a null task ID and `not_created` status, so a wrapper exit cannot erase the attempted run. The Agent Tasks runtime is bundled with this plugin. The helper writes the closed `github.copilot.agent-task-conflict-request` version 1 file and a trusted prompt outside the repository. It loads only the adjacent `cloud_conflict_task.py`, verifies SHA-256 `ecddfa60e8896dfef31f2e441537f04a3814f98c1439f4d25ac65e35b5518412`, and invokes it once with:

```text
--conflict-with-report --strategy <merge|rebase|native-stack> --request-file <absolute-path> --prompt-file <absolute-path> --result-file <absolute-path> --policy marketplace-conflict-worker@6 --pr <canonical-url> --model <alias>
```
...
Policy `marketplace-conflict-worker@6` has SHA-256 `157e03e39720f4d7ef6cab6410f6f866974a1e01fef64cc782474fc04a204b63`.
The bundled worker helper has SHA-256 `ecddfa60e8896dfef31f2e441537f04a3814f98c1439f4d25ac65e35b5518412`.
...
For merge and rebase, the authoritative task artifact branch owns the resolved history. The helper derives the source tip directly from that branch. Native stacks still require one independently assigned, request-scoped ref per role because one branch cannot identify several member tips safely. The authoritative task branch must resolve to the final assigned member tip.
...
The worker does not return summary, validation, commit annotations, conflict paths, companion paths, or rationale as acceptance evidence. The helper derives every SHA, parent, ordered old/new mapping, subject, trailer, path set, patch digest, patch difference, receipt, and result envelope from Git and the frozen request. `github.copilot.agent-task-conflict-receipt` version 3 and `github.copilot.agent-task-conflict-result` version 4 contain no hosted validation schema. A completed task with matching task, session, repository, model, base, and generated-ref identity, no platform error, and valid generated topology is complete. The coordinator does not run local tests or builds. Normal GitHub checks validate behavior after publication.

The coordinator rejects malformed or ambiguous refs, output/code overlap, stale targets, reversed merge parents, extra merge parents, undeclared or reserved paths, dropped, added, squashed, reordered, or nonlinear commits, patch drift outside allowed paths, missing or extra stack members, bad ordering, stale leases, changed outside dependents, and source or base drift. Policy `marketplace-conflict-worker@5` and its report, receipt, semantic output, and result parsers remain immutable audit readers. Older artifacts are never repaired, normalized, or promoted into a current invocation.
