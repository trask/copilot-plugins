...
The helper records `preparing` ownership at the explicit state path before conflict preflight. A preflight error records failed or interrupted ownership with a null task ID and `not_created` status, so a wrapper exit cannot erase the attempted run. The Agent Tasks runtime is bundled with this plugin. The helper writes the closed `github.copilot.agent-task-conflict-request` version 1 file and a trusted prompt outside the repository. It loads only the adjacent `cloud_conflict_task.py`, verifies SHA-256 `c2333f2fa487056c84123e7e3bcf98d8dce1198f6b2d43461b220a74117d264d`, and invokes it once with:

```text
--conflict-with-report --strategy <merge|rebase|native-stack> --request-file <absolute-path> --prompt-file <absolute-path> --result-file <absolute-path> --policy marketplace-conflict-worker@9 --pr <canonical-url> --model <alias>
```
...
Policy `marketplace-conflict-worker@9` has SHA-256 `5fd71b5c27a96a1864bf68995d9fdac1cfbcee4d390f447cc00245537f773a52`.
The bundled worker helper has SHA-256 `c2333f2fa487056c84123e7e3bcf98d8dce1198f6b2d43461b220a74117d264d`.

The full immutable request remains retained outside the repository and is not a worker-accessible file. The hosted problem statement carries every execution identity and commit SHA, plus the complete exact `allowed_paths.paths` array with its count and canonical SHA-256. Workers must read that array before editing. It permits only the listed conflict-resolution patch differences and appended fixes, not whole directories or inferred rename destinations. A required out-of-scope change must stop the worker, not widen the request. Per-commit prompt evidence retains the exact commit SHA, patch digest, and a digest of the complete retained commit evidence.

The managed helper reserves 1,000 characters and UTF-8 bytes below the 28,000-character and 28,000-byte Agent Task limits. It refuses a final policy-wrapped statement over 27,000 characters or UTF-8 bytes as `prompt_too_large` before contacting the Agent Tasks API, including when the complete path set makes it too large. It never samples or truncates permissions, drops other required contract fields to fit, or silently falls back. Exact scope delivery does not establish that an in-scope semantic resolution exists.
...
The coordinator rejects malformed or ambiguous refs, output/code overlap, stale targets, reversed merge parents, extra merge parents, undeclared or reserved paths, dropped, squashed, reordered, or nonlinear replay commits, patch drift outside allowed paths, missing or extra stack members, bad ordering, stale leases, changed outside dependents, and source or base drift. Policy `marketplace-conflict-worker@5`, `marketplace-conflict-worker@6`, `marketplace-conflict-worker@7`, and `marketplace-conflict-worker@8` artifacts remain immutable audit evidence. Older artifacts are never repaired, normalized, or promoted into a current invocation.
