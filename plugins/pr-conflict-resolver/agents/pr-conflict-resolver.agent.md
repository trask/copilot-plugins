...
The helper records `preparing` ownership at the explicit state path before conflict preflight. A preflight error records failed or interrupted ownership with a null task ID and `not_created` status, so a wrapper exit cannot erase the attempted run. The Agent Tasks runtime is bundled with this plugin. The helper writes the closed `github.copilot.agent-task-conflict-request` version 1 file and a trusted prompt outside the repository. It loads only the adjacent `cloud_conflict_task.py`, verifies SHA-256 `d42bea53150d299bab98f0f350f2a05ed24a239a1955fc64e93da752f1f36f45`, and invokes it once with:

```text
--conflict-with-report --strategy <merge|rebase|native-stack> --request-file <absolute-path> --prompt-file <absolute-path> --result-file <absolute-path> --policy marketplace-conflict-worker@3 --pr <canonical-url> --model <alias>
```
...
Policy `marketplace-conflict-worker@3` has SHA-256 `45cf90107f6297a8110a0527ad1f2e80cd02b4b12f71dbfe1fd0a5b48d54a7bb`.
The bundled worker helper has SHA-256 `d42bea53150d299bab98f0f350f2a05ed24a239a1955fc64e93da752f1f36f45`.
...
For merge and rebase, the managed worker writes the resolved code history immediately before one `github.copilot.agent-task-conflict-semantic-output` version 1 commit on the authoritative task artifact branch. The helper requires exact live task, session, base, model, and branch agreement, then derives the code tip only from the semantic commit's sole parent. Native stacks continue to use independently assigned request-scoped member refs because one artifact parent cannot identify multiple member tips. The helper derives every SHA, ordered commit mapping, patch digest, unchanged-path digest, report, receipt, and `github.copilot.agent-task-conflict-result` version 2 envelope from Git and the frozen request. The coordinator rejects missing or malformed semantic output, credentials, malformed refs, artifact/code overlap, incomplete validation, stale targets, reversed merge parents, undeclared merge paths, rebase mapping drift, changed unaffected patches, missing or extra stack members, bad ordering, stale leases, and changed outside dependents. Policy versions 1 and 2 remain legacy evidence only; their artifacts are rejected rather than repaired, normalized, or promoted into a current invocation.
