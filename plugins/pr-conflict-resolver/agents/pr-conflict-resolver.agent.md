...
The managed worker publishes only dispatcher-assigned request-scoped code refs and one `github.copilot.agent-task-conflict-semantic-output` version 1 artifact containing conflict annotations, a summary, and validation evidence. The helper derives every SHA, ordered commit mapping, patch digest, unchanged-path digest, report, receipt, and `github.copilot.agent-task-conflict-result` version 2 envelope from Git and the frozen request. The coordinator requires exact helper, policy, request, result, semantic artifact, receipt, task, model, repository, pull request, and validation identities. It rejects missing or malformed semantic output, credentials, malformed refs, artifact/code overlap, incomplete validation, stale targets, reversed merge parents, undeclared merge paths, rebase mapping drift, changed unaffected patches, missing or extra stack members, bad ordering, stale leases, and changed outside dependents. Policy version 1 results remain legacy evidence only; malformed legacy receipts are rejected rather than repaired or promoted.
