...
PR Pipeline may call `scripts/self_review_loop.py pipeline` with `--bounded-step`, a 32-character lowercase hex `--pipeline-run`, an external `--state` path, and its usual target, iteration, model, and policy arguments. Repeat the same command from the same `COPILOT_AGENT_SESSION_ID` until it returns a terminal result. Each call runs in the foreground. The shared task helper has no aggregate stage deadline; each Git or GitHub CLI subprocess it starts has a five-minute timeout.
...
The completed observation also returns waiting. The next call verifies the result and begins publication. Completed code passes return waiting until a later clean pass or the iteration budget is exhausted.
