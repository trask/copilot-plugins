...
The helper owns authenticated preflight, hosted dispatch, budgets, candidate verification, guarded commit import, exact source publication, and stale-source rejection. Each hosted task performs one review pass and may return several verified code commits. The controller publishes verified fixes and launches a new task to review the resulting head. A clean outcome requires an explicit clean artifact from a pass with no code commits. Exhaustion stays unresolved. Source-only allows verified source publication but forbids PR metadata, reviews, comments, threads, and shared GitHub state changes.
