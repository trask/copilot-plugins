...
- Clear the **Model Gate**, then start the helper's `preflight` workflow at once, with `--new-invocation`. Use a URL or `owner/repo#number` exactly as the user wrote it. For a bare number, combine it with the current workspace's GitHub repository as `owner/repo#number` before you call `preflight`.
...
Clear the **Model Gate** first, then run `preflight`. After `preflight` succeeds, ensure the session name is `Self Review Loop: <PR number> - <PR title>`, built from its `pr.number` and `pr.title` fields. If the harness has already supplied a name beginning `Self Review Loop: <PR number> - `, the name is already correct, so do not call `rename_session`. Otherwise call `rename_session` once with the name you want when the runtime exposes that tool. If the tool is unavailable, or it reports that it skipped the rename because the session already had a name, accept that condition and continue without retrying or reporting it as retrospective friction. Never use an interim number-only name.
...
- Always perform a holistic simplification check after you understand the changed behavior in each area and then review the PR as a whole. Ask whether a broader structural change, such as consolidating duplicate paths behind a shared helper, moving a responsibility to a clearer boundary, simplifying an API, or removing coordination between related changes, would make the overall PR simpler. Check this both as a better fix for an individual finding and as a distinct whole-PR candidate that combines related changes. Register either candidate only when the diff, code context, or a strong precedent provides concrete evidence and the change fits the PR's stated scope; do not create a commit for a generic refactor preference or unrelated redesign.
- Never raise a finding again when the carried-forward `history` already records it as `dropped`, `addressed`, or `no_code` and the current head retains that decision. An intentional revert of this loop's fix is the author rejecting the finding, so do not restore it or register the same root cause again. A rebase or force-push that loses the patch without evidence of an intentional revert is different: follow `history_commit_presence`, inspect the current code when the helper cannot prove retention, and review the finding again only when the fix is actually gone.
...
- `preflight [target] [--repo-root <workspace>] [--max-iterations 5] [--new-invocation | --invocation-run <token>]`: resolve and check out the PR, require a clean worktree, realign a force-pushed PR branch safely and only when `git cherry` proves the local commits hold no unique patches, require the local head to equal the PR head, fetch and parse the authoritative diff, confirm the head did not move around that fetch, enforce the active invocation's iteration cap, archive the previous iteration, compare every commit recorded in history with the current PR commits, discover tracked repository guidance and validation sources around the PR-authored paths, write its complete result to `preflight_path` as JSON, and print only a compact envelope carrying `result`, `state`, `preflight_path`, `repo_root`, PR identity, `head_sha`, `diff_path`, `diff_bytes`, `body_bytes`, `counts`, `iteration`, and `max_iterations`. The complete result at `preflight_path` adds the complete PR body and metadata, `changed_files`, GitHub's ordered `pr_commits` with each commit's touched `files`, their `pr_authored_files` union, `diff_only_files`, the carried-forward `history`, `history_commit_presence`, and `repository_context`. Each `history_commit_presence` entry reports `commit_match_kind` for an exact commit, an equivalent whitespace-preserving patch, a missing patch, or unknown identity. Its separate `retained` field is true only when the helper can reverse the recorded patch from the final pinned worktree and cannot apply it, false only when the forward check succeeds and the reverse check fails, and null when both or neither check succeeds. The helper never equates two commits whose patch identity is absent and never treats commit-list presence alone as final-tree retention. The envelope keeps `counts.history_commits_missing` for exact-SHA reporting and adds counts for unmatched and unknown fixes plus the discovered context. `cleanup` deletes the result along with the state and diff files.
...
- `resolve --state <path> --outcome clean`: require that there are no candidates, or only dropped ones; return `head_moved` and supersede the stale review when the live PR head no longer matches the pin, so its dropped candidates do not enter history; otherwise durably mark the active review clean
...
The helper's current result decides the transition. Never jump to a later stage because stale prose, a previous result, or the working tree looks ready.
...
| Planning and editing | validation, then commit, then `record` | An unrelated dirty path stops the run. A correctable plan repeats `plan` before editing. |
| Validation | commit every required rewrite, then `publish` | An unrecoverable failure enters `skip` and terminates without a partial push. |
| `publish` | `published` returns to `preflight` | `nothing_to_publish` is terminal. A helper error stays at `publish` and preserves its state. |

Apply precedence in this order: the helper's head and state guards, **Non-Negotiable Rules**, the current-stage rules in this table, the closest applicable repository instruction, then general repository guidance. A closer repository instruction may specialize a broader one, but it cannot override this workflow's safety guards. When two rules at the same level contradict each other and the repository does not resolve them, stop rather than choose silently.

## Repository Context And Validation Map

Read `repository_context` from `preflight_path` before review. It is a dependency-free discovery index over tracked files, not a claim that unlisted guidance or commands do not exist. It groups PR-authored paths by nearby instruction files and build manifests, and separately lists repository instruction files, knowledge articles, and GitHub Actions workflows.

For each changed area, read the closest applicable instruction files first. Read the discovered knowledge files whose names or contents bear on the change. Follow links to the source that owns a convention instead of reconstructing it from history or sibling code. If a candidate depends on a convention, give the evaluator the owning instruction or state plainly that no repository instruction establishes it.

Build one private validation map before evaluation. For each PR-authored path or path group, record the exact command, working directory, prerequisites, files the check reads, expected cost, and the source that established the command. Mark CI evidence for the pinned head separately from a command you can run locally. Reuse this map when you plan batches and clear **Local Validation Before A Push**. The discovered files are a starting point, not an outer bound: search further when they do not settle a candidate or provide a usable narrow check. A missing narrow command still follows the publish-with-`--not-validated` rule below.

## Holistic Simplification Check

After you understand the changed behavior and its surrounding code, check each changed area and then the PR's combined design for a simpler structure. Look for duplicated control flow or configuration, abstractions that force callers to compensate, responsibilities split across the wrong boundary, coordination that exists only because related changes were designed separately, and fixes that add exceptions instead of removing the underlying complexity. Compare the PR with the closest existing implementations and shared helpers before deciding that a broader change is warranted.

A broader simplification may be either a better fix for an individual finding or a distinct whole-PR candidate that makes related changes simpler together. In both cases it needs a concrete benefit for the overall design, evidence from the diff, code context, or a strong precedent, and a fit with the PR's stated scope. It may require more lines or files than a local fix, and the resulting batch may touch those additional paths, but every edit must remain directly necessary for the proposed simplification. Do not turn a general preference for refactoring, a speculative redesign, or unrelated cleanup into a finding.
...
5. Run `preflight` once per iteration. The helper may realign the clean PR branch after a force-push, but only when it proves the local commits hold no unique patches. If it reports `head_moved`, stop on that exact error. Never stash, reset, discard, or force local work by hand to make preflight pass.
...
Record the returned `head_sha` as the immutable snapshot for this iteration, and do not replace or refresh it. Read the pinned diff only from the returned `diff_path`, which holds the exact text the helper fetched and validated at that head. Never run `gh pr diff` again and never rebuild the changeset another way. Read the full `pr`, `changed_files`, `pr_commits`, `pr_authored_files`, `history`, `history_commit_presence`, and `repository_context` from the complete result at `preflight_path`, paging through it with explicit line ranges when it exceeds a read tool's size limit, and check what you read against the envelope's `counts` so you skip nothing.

Treat a history entry as settled when it has no commit or `history_commit_presence.retained` is true. `commit_match_kind` explains whether the same commit or patch exists in the PR history, but that alone does not prove a later commit kept the fix in the final tree. When `retained` is null, inspect the pinned diff and current code. Keep the entry settled when the fix remains or when an intentional revert shows that the author rejected it. Review the finding again only when `retained` is false, or inspection proves the fix is gone, and no intentional rejection settles it. Do not compare commit lists or reconstruct patch identity by hand.
...
1. Read the pinned PR title and description plus `repository_context` from `preflight_path`. Build the instruction index and validation map under **Repository Context And Validation Map**, complete **Holistic Simplification Check**, then read only the additional context you need to understand the changed behavior. The pinned body establishes this iteration's scope. **PR Metadata Accuracy** still requires a fresh live title and body after publication and before the terminal response.
2. Review the whole pinned diff read from `diff_path`, including commits this loop created in earlier iterations. Read the whole pinned diff on the first iteration, and whenever the head holds any change this run did not publish. On an iteration that directly follows a publication, where the new preflight head equals the head the preceding `publish` returned and that publication proved the only new commits were this loop's recorded commits, carry the earlier full review forward and review only those newly published commits in their current pinned-diff context; you do not need to read unchanged hunks again. That incremental pass satisfies the whole-diff rule, because the earlier review plus the exact proven delta covers every line of the current pin. Build a private candidate list. Give each entry an exact path, a changed line, `LEFT` for a deleted line or `RIGHT` for an added line, its demonstrated impact, and a plain few-sentence description of the problem and of one concrete fix. A changed line may anchor a defect whose complete cause or fix also involves unchanged code only when that changed line genuinely demonstrates the defect or incomplete fix. Never choose an unrelated changed line merely because the helper accepts it. Before you keep a candidate that claims a semantic or convention violation, read the implementation or the authoritative documentation of any shared helper that defines that contract, and confirm the candidate's premise. Do not send an assumption to the evaluator when one direct read of that helper can disprove it.
...
8. An evaluator may improve the proposed fix without you registering a new candidate, as long as the registered defect stays factually correct and the improved fix addresses that same demonstrated root cause. The registered anchor identifies the defect, not the largest edit you may make. The improved fix may touch more lines or more files, including lines the PR already changed, but only when each edit is directly necessary for that root cause and stays within the PR's scope. A whole-PR simplification candidate may instead combine related changes when every edit is directly necessary for that proposed simplification and the evaluator supports the combined design with evidence. Do not absorb a separate defect just because the evaluator noticed it; drop that separate issue or leave it for later instead.

Whenever either clean-exit `resolve` call reports `head_moved`, discard the old snapshot and start again with a fresh `preflight`. The helper supersedes that stale review so its dropped candidates do not enter history. No publication occurred, so this does not consume an iteration.
...
The general pull request instruction to keep the title and description materially accurate applies at every terminal outcome, whether the inaccuracy predates this loop or a loop commit introduced it, and it takes precedence over the normal push-only limit on changes. After each successful `publish`, and before the next `preflight`, read the live title and description again against the newly published diff. Update only the part that is materially false or misleading, at once, using the mechanism the general pull request instructions prescribe, and keep the author's intent and the context that helps. Check once more before the terminal response, and correct any material inaccuracy against the final diff. Do not edit metadata just to record validation, a minor implementation detail, or an incidental change, and never turn the correction into a review or a PR comment. If you cannot make a required metadata correction safely, stop rather than finish with metadata you know is inaccurate.

## Final Response

Keep chat as a compact index, because the reasoning for accepted findings lives in git. Emit exactly one terminal response and make it the last message of the run. Render ordinary Markdown, never a fenced code block. Emit one linked list item per commit, then any no-code outcome, one loop-outcome line, an optional not-validated-locally line, an optional PR-metadata line when this run corrected the title or description, an optional dropped-candidate block, and finally the canonical pull request link from the most recent preflight result's `pr.pr_url`:
