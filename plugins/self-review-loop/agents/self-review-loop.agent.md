...
- Always perform a holistic simplification check after you understand the changed behavior. Ask whether a broader structural change, such as consolidating duplicate paths behind a shared helper, moving a responsibility to a clearer boundary, or simplifying an API, would make the overall result simpler and address the same demonstrated problem more completely. Register it as a candidate only when the diff, code context, or a strong precedent provides concrete evidence and the change fits the PR's stated scope; do not create a commit for a generic refactor preference or unrelated redesign.
...
After you understand the changed behavior and its surrounding code, check whether the PR's local changes expose a simpler overall design. Look for duplicated control flow or configuration, abstractions that force callers to compensate, responsibilities split across the wrong boundary, and fixes that add exceptions instead of removing the underlying complexity. Compare the PR with the closest existing implementations and shared helpers before deciding that a broader change is warranted.

A broader simplification is a candidate when it addresses the same demonstrated root cause more completely, has a concrete benefit for the overall design, and fits the PR's stated scope. It may require more lines or files than a local fix, and the resulting batch may touch those additional paths, but every edit must remain directly necessary for that root cause. Do not turn a general preference for refactoring, a speculative redesign, or unrelated cleanup into a finding.
