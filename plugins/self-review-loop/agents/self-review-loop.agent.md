...
- Stop on every coordinator error. Report the returned state path, task ID status, task URL or ID, generated branch and head, ordered fix commits, validation and report paths, retained recovery files, and exact `recovery_command` or `retry_command`. Run that command only when the user asks. A trusted task-creation failure with no task ID uses the fresh retry command after its prerequisite is fixed; it must not use `--resume`.
