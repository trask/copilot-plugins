...
The worker owns all repository analysis and execution. It diagnoses the exact observed failures, makes the edits, adds or updates tests, formats the changes, runs relevant builds and tests, and emits linear fix commits followed by one human-readable report commit. It never sleeps, polls, watches, waits for CI, or starts another iteration.
