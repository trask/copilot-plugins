...
- The maximum is 5 iterations, and an iteration is charged per head rather than per launch. Reading the checks again at the head this loop already spent an iteration on, whether after a re-run or simply because you started over, costs nothing; only moving the head to a new commit spends the next one. Respect `max_iterations_reached` before you edit anything; do not work around it. An orchestrator running this loop as one stage of a larger loop may raise that budget, but only by naming its own position, never by anything this loop notices about itself.
...
- `waiting`: the helper only returns this when you asked it not to wait. Run it again with `--wait`.
