...
- `stack-format --state <path> (--format-command <executable> [args...] | --no-format)`: complete the pending propagation formatting checkpoint through PR Conflict Resolver. Use the repository's documented formatter, such as the affected Spotless task, or `--no-format` only when the repository has no formatting step. Repeat while the result is `format`; when it returns `formatted`, return to `stack-next` so the coordinator can resume and publish the preserved propagation workspace.
