...
- `wait --state <path> --stage <name> --pid <id> [--process-create-time <t>] [--timeout <s>] [--poll <s>]`: wait for at most one five-minute slice. It returns `finished` with an `outcome` only after the whole stage process tree has exited, so the next stage never launches into the shared worktree while this one is still pushing. A live active process tree returns `still_running`; call `wait` again. A process tree that exits without an outcome returns `carry` with reason `process_exited_without_outcome`, so the stage is carried to the next pass rather than ending the run. The pipeline watches helper-state changes, stage-log growth, process-tree changes, and root/descendant CPU progress. Only 30 minutes with none of those terminates the process tree and escalates as `stage_inactive`; there is no absolute stage wall-clock ceiling. If termination cannot be confirmed, the escalation remains terminal and a later preflight refuses to reuse the worktree until that recorded tree is gone. Pass the `process_create_time` from `launch` so a reused pid cannot read as alive.
...
6. Wait for the stage with `wait --stage <plan stage> --pid <the pid> --process-create-time <the value launch returned>`. It returns within about five minutes. On `still_running`, run the same `wait` command again; active stages may run indefinitely. On `finished`, take its `outcome`. On `carry`, the process died before the stage recorded anything: carry the stage with `finish --stage <plan stage> --outcome carried --carried-reason process_exited_without_outcome`, then go back to step 1. On `escalate`, go to **Escalation**: `stage_inactive` means the alive process tree showed none of the four observable activity signals for 30 minutes, so `wait` terminated it and recorded the terminal escalation without resetting or pushing anything.
