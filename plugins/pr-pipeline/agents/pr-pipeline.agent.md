...
- `wait --state <path> --stage <name> --pid <id> [--process-create-time <t>] [--timeout <s>] [--poll <s>]`: use one fixed five-minute slice that activity cannot extend. While the Copilot root is alive, a recorded outcome does not finish the stage because it may still be publishing. Once that root is confirmed exited, a terminal helper outcome wins immediately over remembered descendants, so reusable build daemons cannot pin a completed stage. Without a terminal outcome, a real surviving descendant keeps the stage running and preserves crash recovery. A fully exited process tree with no outcome returns `carry` with reason `process_exited_without_outcome`. The pipeline watches helper-state changes, stage-log growth, process-tree changes, and root/descendant CPU progress. Only 30 minutes with none of those terminates the process tree and escalates as `stage_inactive`; there is no absolute stage wall-clock ceiling. If termination cannot be confirmed, the escalation remains terminal and a later preflight refuses to reuse the worktree until that recorded tree is gone. Pass the `process_create_time` from `launch` so a reused pid cannot read as alive.
...
6. Wait for the stage with `wait --stage <plan stage> --pid <the pid> --process-create-time <the value launch returned>`. Its five-minute slice never resets when activity appears. On `still_running`, run the same `wait` command again; active stages may run indefinitely. On `finished`, take its `outcome`. On `carry`, the process tree exited before the stage recorded anything: carry the stage with `finish --stage <plan stage> --outcome carried --carried-reason process_exited_without_outcome`, then go back to step 1. On `escalate`, go to **Escalation**: `stage_inactive` means the alive process tree showed none of the four observable activity signals for 30 minutes, so `wait` terminated it and recorded the terminal escalation without resetting or pushing anything.
