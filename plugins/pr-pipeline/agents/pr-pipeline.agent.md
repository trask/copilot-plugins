...
The helper invokes each installed Python coordinator directly, not through a model session. Each stage waits for its children and owns one configured iteration allowance for the entire run. Sweeps never reset or multiply it. A nonzero stage exit, missing or unreadable state, or an active child after its coordinator returns blocks the Pipeline. An interruption abandons the run; start from the beginning rather than resuming. The helper runs a second sweep when the pull request head or base changed during the first and some stage is not clear at the final revisions. On unchanged revisions, it also uses the remaining sweep when the sole uncleared stage is CI and fresh verification shows that its green or warning snapshot changed. No other unchanged-stage result starts another sweep. A completed PR Conflict Resolver run is not launched again during that pipeline run.

Hosted workers write under `.github/agent-task-output/`. Optional `report.md` is advice, never stage evidence. Self Review requires an outcome and consumed-pass count, Review requires finding dispositions and commit indexes, CI may require diagnoses, and Description requires `title.txt` and `body.md`. Their coordinators validate these semantic inputs separately from dispatcher provenance. Do not interpret worker output locally or treat a candidate or semantic claim as CI green. A proven source-drift result remains uncleared and keeps its spent allowance; the helper retains it without adopting or publishing the stale candidate, and only an already-authorized later sweep may clear the current snapshot.
