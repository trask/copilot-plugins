...
Windows launch requires breakaway and fails before fallback if denied. The receipt records flags and process generation. Watch checks that generation; missing, unreadable or reused identity is a monitoring failure. Never infer independence from no-window flags or infer workflow completion from process exit.
