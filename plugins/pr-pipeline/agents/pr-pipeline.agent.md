...
- **Never read a stage's prose report to make a decision.** A stage's summary is for the user. Every control-flow decision comes from `next`, which reads the stage helpers' `status` subcommands and live GitHub state.
...
8. Record it with `finish --stage <name> --outcome <outcome> --head <the head after the stage ran> [--detail <one plain sentence>]`.
9. Apply **Session Hygiene**.
10. Go back to step 1.

Reading a stage's report to fill in `--detail` is fine. That text is for the user. Never let it choose the outcome when `next` and the live state say otherwise.
