...
Never end your turn or leave the session idle while `finished` is false. Stop only when `finished` is true. On a normal terminal update, use its `final_event` as the complete `pipeline_finished` result. A `monitor_failure` is terminal for monitoring and has no `next_watch`; report it without retrying or guessing the pipeline outcome. Progress reporting is deliberately separate from scheduler execution.
