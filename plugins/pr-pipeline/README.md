...
When a later Review commit advances the same head branch, CI status reports its old green or warning clearance as stale without fetching checks for the obsolete head. It confirms the new commit descends from the saved head and keeps branch identity and base fixed. Pipeline can then run CI at the new revision; a rewritten head or changed branch remains an identity error.
