...
The scheduler runs these stages in order. A second sweep starts when the head or base changed during the first sweep and at least one stage is not clear at the final revisions. On unchanged revisions, it also starts when the sole uncleared stage is CI and fresh verification shows that its green or warning snapshot changed; no other unchanged-stage result starts another sweep. Conflict Resolver pins the base for hosted work, accepts a forward advance of the live target branch, and checks mergeability at the verified PR head. A rewritten base still blocks publication. A later sweep can refresh incomplete mergeability or CI snapshot evidence.

Every stage is an installed Python coordinator. The PR Pipeline custom agent calls `start` once and then calls `advance` in the same Copilot session until the run finishes. A local stage step has no elapsed-time deadline: it runs to a sealed result or explicit cancellation, then returns `continue`, `waiting`, or a final result. Conflict Resolver can prepare a native stack before dispatching a hosted task. When hosted work is active, the stage records its exact identity and the agent makes a later observation call; no local process needs to wait for the hosted task. No model translates a stage's command or exit status into clearance. Copilot Review, Self Review, and CI Fix each get their configured iteration allowance per sweep; relaunching within the same sweep does not replenish it. A nonzero exit or unfinished child blocks the Pipeline even if a clearance marker exists. An interrupted session leaves its run incomplete; a later session starts a fresh run, not an automatic continuation.

The head repository's live `refs/heads/<head-branch>` tip is the source for every stage, not the caller's checked-out branch or a PR commit pointer that may lag a native restack. Single-PR Pipeline requires a clean worktree, fetches that branch from its repository (including a fork), verifies the fetched commit against the current remote ref, and checks it out. It derives published commit history from the fetched source and base commits rather than the PR commits endpoint. Before leaving a local tip that is not an ancestor of the fetched head, it retains that tip at `refs/copilot/pr-pipeline/<run-id>/<pr-number>/<sha>` and reports the ref in `checkout_head_retained`; a failed retention blocks checkout. A named local branch is left unchanged. The retained tip is recoverable but does not become part of the published PR, and a stage that leaves unpublished commits or a dirty worktree still blocks. Stack Pipeline resolves each open member's head repository and live branch ref before freezing its source snapshot and creating owned worker worktrees; inactive prefixes retain historical commit IDs without looking up deleted branches. A stale PR commit pointer cannot certify conflict status; a complete stack can instead clear from its verified Conflict Resolver receipt. Partial suffixes still require a passing conflict condition because they cannot authorize publication for unselected ancestors.

Verified Review exhaustion is terminal `carried`, not clean. Pending feedback and spent allowance remain in its status while Self Review, CI and Description continue. A later sweep can address pending feedback with its own allowance. The final result stays incomplete or partial while Review remains unresolved.
...
On a later sweep of the same run, each stage keeps its pull request and run ownership, completed task receipts, and spent allowance. It fetches the current PR and checks clearance against that new snapshot. A changed head, base, branch, draft state, title, body, checks, or review feedback does not have to match the preceding sweep. Pending hosted tasks still use their pinned input, and stale clearance never applies to the new snapshot.

On a later native pass, an unchanged member reuses Description only when this run already collected its accepted exit-zero completion and its recorded snapshot is still current. The invocation, model, mutation policy, and earlier pass must match. Other members whose heads advanced still run Description normally. A later single-PR sweep can evaluate Description again, even on the same head, when its old clearance is stale. Phase and terminal results list reused members separately from dispatched workers and accepted completions, including when all members reuse clearance. A failed worker exit always blocks even if its state retains an older clear marker.
