...
- Run discovery through **Iterative Discovery**. Each pass uses a fresh GPT-5.6 Sol subagent and receives the accumulated exclusion ledger. Never let a pass stop after rediscovering a known finding, and never evaluate or post until discovery reaches a valid clean pass or the five-pass cap.
...
| --- | --- | --- |
| Model gate | The agent starts | A GPT-family model is confirmed, or the run stops for an override |
