...
- A finding is suggestion-eligible whenever its complete fix can be expressed as an exact replacement for the anchored changed line. Every suggestion-eligible comment must include a fenced GitHub `suggestion` block containing that full replacement line; a prose-only comment is invalid and must not be posted. Repeat the complete line when only one cell or token changes, including for Markdown table rows. Use prose-only guidance only when the correct replacement requires an author decision or cannot be encoded by replacing the anchored line.
...
7. Rewrite every surviving comment to **Comment Style**, then recheck it for a valid changed-line anchor and correct `LEFT`/`RIGHT` side. For each comment, explicitly determine whether the complete fix is an exact replacement for that anchored line. If so, require a fenced GitHub `suggestion` block with the full replacement; do not accept prose as a substitute. Otherwise require concrete prose guidance. Write the structured array to a short-lived local file or pass it on standard input.
