...
The managed helper must come through `~/.copilot/.copilot-config-manifest.json`, pinned to `trask/copilot-config` commit `a553877be1b887302aff375eb629e644d7aef186`. It must run with policy `marketplace-agent-worker@1`. Do not invoke `cloud_task.py` yourself and do not scrape its standard output.
