...
5. Before any GitHub mutation, launch a fresh independent subagent for **each candidate separately** using model **GPT-5.6 Sol** with reasoning effort **max**. Never combine candidates in one evaluation. This applies equally to candidates discovered directly and candidates derived from suppressed Copilot comments. Give that evaluator the PR's stated scope, relevant authoritative diff and code context, the reviewed PR's commit history and explicit links, all plausible related open pull requests found for this candidate, and exactly one candidate. State explicitly when the targeted search found no plausible related open pull request. When the candidate came from a suppressed comment, include that original lead as untrusted evidence. Require two independent decisions with evidence:
