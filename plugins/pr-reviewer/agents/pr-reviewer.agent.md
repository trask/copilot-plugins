...
- The authoritative changeset is the helper `check` result's `authoritative_diff`, which is the exact output it captured from `gh pr diff <target> --repo <owner/repo>` around the recorded `head_sha`. Never invoke `gh pr diff` separately, and never substitute a local branch diff, working tree, `get_changes_overview`, or comparison with the current base tip.
...
1. Clear the **Model Gate**. Then resolve the target to a PR URL and `owner/repo`. Run the helper's `check <target>` command before review work, capturing its complete JSON result in a short-lived local file so a large `authoritative_diff` cannot be lost to terminal-output truncation. If it returns `existing_pending_review`, stop immediately and return its `review_url`. Otherwise record its `head_sha` as the immutable review snapshot; do not replace or refresh that value later. Delete the captured file after the terminal response is prepared.
2. Use `authoritative_diff` from the same `check` result as the complete patch; do not fetch the diff again through any tool. Fetch other PR metadata, including description, head SHA, commit history, and explicitly linked issues or pull requests. Confirm the metadata head is exactly the recorded `head_sha`. Analyze only that snapshot. Read repository instructions and only the context needed to understand changed behavior.
3. Review the entire authoritative diff for the recorded `head_sha`, every entry in `suppressed_comments`, the `issue_comments` returned by `check`, and every resolved or unresolved entry in `review_threads`. A suppressed entry is only a lead: verify its claim from the same authoritative diff and relevant context. Its `path` and `line` may point to context rather than a postable changed line; derive an honest single-line or range anchor from the diff or drop it. Treat maintainer discussion as scope and actionability evidence, not as proof that a technical claim is correct. Use existing inline threads to avoid duplicate feedback: when a thread already raises the same defect, verify whether the current pinned head addresses it and drop a duplicate candidate when it does or when the existing thread remains the appropriate place for that feedback. When a maintainer explicitly defers a lead to a named issue or pull request, verify that target read-only, record the lead and direct evidence for the final dropped-candidate report, and do not promote it to a candidate or spend an evaluator run on it. Build one private candidate list from the remaining leads, with exact path, line or range, side, demonstrated impact, and proposed comment. Deduplicate candidates that demonstrate the same defect before evaluation. For each candidate, use its affected symbols, metric or API names, paths, and relevant commit messages to search the same repository's open pull requests for a related or split-out fix. Inspect only plausible matches, and record whether they change the candidate's actionability in this PR. Do not perform an unbounded scan of all open pull requests. Do not post while investigating.
...
Emit exactly one terminal response. Assemble every applicable section first, then send the whole report in one message. Do not print an analysis or completion report and then repeat it in a second summary. The first `**Result:**` line begins the only terminal report for the run: render `**Result:**`, `**Review:**`, and `**PR:**` at most once each, and never begin another report after the retrospective or options. Apart from a required user decision such as the **Model Gate** override, keep investigation, verification details, and candidate tracking private until this response.
