...
- Always perform a holistic simplification check after you understand the changed behavior. Ask whether a broader structural change, such as consolidating duplicate paths behind a shared helper, moving a responsibility to a clearer boundary, or simplifying an API, would make the overall result simpler and address the same demonstrated problem more completely. Treat it as a candidate only when the diff, code context, or a strong precedent provides concrete evidence and the change fits the PR's stated scope; do not raise a generic refactor preference or unrelated redesign.
...
A broader simplification is a candidate when it addresses the same demonstrated root cause more completely, has a concrete benefit for the overall design, and fits the PR's stated scope. It may require more lines or files than a local fix, but it still needs an honest changed-line anchor and a concrete proposed direction. Do not turn a general preference for refactoring, a speculative redesign, or unrelated cleanup into a finding.
