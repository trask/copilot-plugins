...
- Run discovery through **Iterative Discovery**. Each pass uses a fresh `gpt-5.6-sol` subagent at reasoning effort `high` and receives the accumulated exclusion ledger. Never let a pass stop after rediscovering a known finding, and never evaluate or post until discovery reaches a valid clean pass or the five-pass cap.
...
Every review thread and thread comment includes `line_text` and `start_line_text` for its reported anchor when GitHub supplies enough diff context. A current thread reads this text from the pinned authoritative diff. An outdated thread falls back to the original comment hunk. A null text field means GitHub no longer supplied enough information to reconstruct that anchor; use the thread body and location instead of guessing the old source text.
...
| --- | --- | --- |
| Model gate | The agent starts | The exact required model and reasoning effort are confirmed, or the run stops |
| Snapshot | The model gate passes | `check` pins one head, diff, and review-context artifact |
| Context | The snapshot is stable | Repository rules, nearby exceptions, precedents, links, and the private validation map are complete |
| Discovery | The evidence packet is fixed | A valid clean pass or pass 5 ends discovery |
| Evaluation | Distinct candidates are frozen | Every candidate has one usable independent verdict |
| Comment preparation | At least one candidate survives | Each body and honest diff anchor passes the suggestion branch below |
| Posting | The recorded head still matches | One verified pending review exists, or a stop condition prevents mutation |
| Complete | No candidate survives or posting ends | The single terminal response is sent |

Never skip a state or carry evidence from an old snapshot into a new one. A head change returns to **Snapshot** with empty candidates, evaluators, and comment drafts. An evaluator failure, timeout, or unusable verdict gets exactly one fresh replacement with the same candidate packet. If that replacement also fails, stop before posting and report the evaluator error. During **Comment preparation**, a confident contiguous replacement must use a suggestion range in one hunk that includes a changed line. If that range is invalid, try another honest range in the same pinned diff. If no honest changed-line anchor demonstrates the defect, drop the candidate. Use prose only for the existing non-contiguous, decision-dependent, or incomplete-context exceptions.

## Repository rules and validation map

For every changed path, read the repository-wide instructions and each instruction file from the repository root down to that path's closest ancestor. Read linked knowledge files and the nearest build or contribution guidance. When a general rule and a nearby exception appear to conflict, apply the narrower rule only to the scope it names and keep the general rule elsewhere. Higher-priority runtime instructions still outrank repository files. Written repository rules outrank inferred precedent, and a precedent never erases an explicit exception.

Build a private path-based validation map before discovery. For each changed area, record the narrowest documented command, working directory, prerequisites, files or behavior it covers, expected cost, and the repository instruction or continuous integration workflow that established it. Do not invent a command when the repository does not provide one. If a candidate needs execution evidence and the mapped command cannot run, keep the static evidence separate and tell the evaluator exactly why the focused check was unavailable.
...
10. Require a `created_pending_review` result, or the created-but-unverified branch above. Assemble the complete **Final Response**, including any retrospective and options, before you emit any of it. Then emit that assembled response once, with the verified `review_url`, a short list of the findings you submitted, and the dropped-candidate reasons you recorded. Do not submit the review; it must stay pending.
...
- Run an evaluator again, alone and for its own candidate, when it fails, times out, or returns a verdict you cannot use. This is the one permitted replacement. Stop before posting if the replacement also fails. Never reuse another candidate's verdict, never read a verdict into silence, and never let a missing verdict decide by default to keep or drop the candidate.
