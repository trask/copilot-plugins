...
Body feedback includes legacy `Suppressed comments` sections and CCR v2 `Previously missed` sections with nested findings. The coordinator extracts their structural locations and complete finding text, removing zero-width display separators from paths without judging actionability. Every extracted finding goes to the hosted worker. A recognized feedback section that cannot be completely parsed fails explicitly instead of recording a clean review. Overview prose and `Resolved since last review` sections are not finding records. Body-only findings never receive a reply or thread resolution.
