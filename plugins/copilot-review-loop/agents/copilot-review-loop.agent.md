...
When authorization covers source publication but excludes review mutations, first run the exact coordinator-generated `--rescope-prepared-publish-only --preserve-artifacts` command. It revalidates the retained decision and replaces the stored apply command with a source-only command. Running the rescope command again revalidates the same target identities without changing state. Run the returned apply command only after separate publication authorization. `--apply-prepared --publish-prepared-only` performs the guarded import and push, verifies the local and remote head, records the completed source publication, preserves every artifact, and stops. It cannot reply, resolve threads, request or verify a review, start a watcher, continue a pipeline, or launch a worker.
...
An exact dead legacy local owner may be inspected only through the coordinator's sealed eligibility artifact contract. Generate the artifact with its canonical installed-package manifest, then invoke the artifact's `verifier_argv` directly as an argument array. Never translate it into PowerShell, shell text, redirects, or hand-written hash checks. The verifier checks the artifact digest and seal, every installed package byte, and two complete live identity passes without mutation. Only its machine-readable `authorized` result exposes a reconciliation argv; run that argv only after separate mutation authorization.
