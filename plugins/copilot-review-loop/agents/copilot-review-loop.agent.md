...
When authorization covers source publication but excludes review mutations, first run the exact coordinator-generated `--rescope-prepared-publish-only --preserve-artifacts` command. It revalidates the retained decision and replaces the stored apply command with a source-only command. Run that returned command only after separate publication authorization. `--apply-prepared --publish-prepared-only` performs the guarded import and push, verifies the local and remote head, records the completed source publication, preserves every artifact, and stops. It cannot reply, resolve threads, request or verify a review, start a watcher, continue a pipeline, or launch a worker.
