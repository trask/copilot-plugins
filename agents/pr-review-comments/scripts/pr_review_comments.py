#!/usr/bin/env python3
"""Deterministic mechanics for the PR Review Comments custom agent."""
...
TARGET_PATTERN = re.compile(
    ...
    r"(?:#(?:(?:pullrequestreview-(?P<review>\d+))|(?:discussion_r(?P<comment>\d+))))?/?$"
)
...
def parse_target(target: str) -> dict[str, Any]:
    ...
    if not match:
        raise WorkflowError(
            "target must be a GitHub PR, review, review-comment URL, or owner/repo#number"
        )
    ...
    if values.get("review"):
        scope_type = "review"
        scope_id = int(values["review"])
    elif values.get("comment"):
        scope_type = "comment"
        scope_id = int(values["comment"])
    return {
        "owner": values["owner"],
        "repo": values["repo"],
...
def default_state_path(target: dict[str, Any]) -> Path:
    ...
    return Path.home() / ".copilot" / "run" / "pr-review-comments" / name
...
def select_queue(
    threads: list[dict[str, Any]], scope_type: str, scope_id: int | None
) -> list[dict[str, Any]]:
    queue: list[dict[str, Any]] = []
    for thread in threads:
        if thread["isResolved"]:
            continue
        candidates = thread["comments"]["nodes"]
        if scope_type == "review":
            candidates = [
                comment
                for comment in candidates
                if comment.get("pullRequestReview", {}).get("databaseId") == scope_id
            ]
        elif scope_type == "comment":
            candidates = [
                comment for comment in candidates if comment.get("databaseId") == scope_id
            ]
        else:
...
def metadata_for(target: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "head_owner": metadata["headRepositoryOwner"]["login"],
        "head_repo": metadata["headRepository"]["name"],
...
def command_preflight(args: argparse.Namespace) -> None:
    ...
    comments = select_queue(threads, target["scope_type"], target["scope_id"])
    queue_id = (
        f"{target['scope_type']}-{target['scope_id']}"
        if target["scope_id"] is not None
        else f"pr-{target['number']}"
    )
    state = prior_state or {"version": STATE_VERSION, "created_at": utc_now(), "queues": {}}
    state.update(
        {
    ...
    emit(
        {
            "result": "ready" if comments else "no_unresolved_comments",
...
def active_queue(state: dict[str, Any]) -> dict[str, Any]:
    queue_id = state.get("active_queue")
    if not queue_id or queue_id not in state.get("queues", {}):
        raise WorkflowError("state has no active queue")
    return state["queues"][queue_id]
...
def find_comments(queue: dict[str, Any], ids: Iterable[int]) -> list[dict[str, Any]]:
    ...
    if missing:
        raise WorkflowError(f"comments are not in the active queue: {missing}")
...
def command_refresh(args: argparse.Namespace) -> None:
    ...
    queue = active_queue(state)
    comments = find_comments(queue, args.comments)
    threads = fetch_threads_by_id(comment["thread_id"] for comment in comments)
    current_by_id = {
...
def reply_body(comment: dict[str, Any]) -> str:
    if comment.get("commit"):
        return f"Addressed in {comment['commit']}: {comment['summary']}"
    return f"No code change: {comment['rationale']}"
...
def resolve_threads(comments: list[dict[str, Any]]) -> None:
    thread_ids = list(dict.fromkeys(comment["thread_id"] for comment in comments))
    if not thread_ids:
        return
    fields = " ".join(
...
def queues_to_publish(state: dict[str, Any], all_queues: bool) -> list[dict[str, Any]]:
    if all_queues:
        return [
            queue
            for queue in state["queues"].values()
            if any(comment["status"] == "handled" for comment in queue["comments"])
        ]
    return [active_queue(state)]
...
def verify_publish(state: dict[str, Any], comments: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    threads = fetch_threads(pr["upstream_owner"], pr["upstream_repo"], pr["number"])
...
def command_publish(args: argparse.Namespace) -> None:
    ...
    if dirty:
        raise WorkflowError(f"worktree is not clean:\n{dirty}")
    queues = queues_to_publish(state, args.all_queues)
    comments = list(
        {
            comment["id"]: comment
            for queue in queues
            for comment in queue["comments"]
            if comment["status"] == "handled"
        }.values()
    )
    if not comments:
        if not args.no_comments:
            raise WorkflowError("there are no handled comments in the publishing scope")
        if args.all_queues or any(queue["comments"] for queue in queues):
            raise WorkflowError("--no-comments requires an empty active queue")
    incomplete = [
        comment["id"]
        for comment in comments
    ...
    emit(
        {
            "result": "published",
            "state": str(path),
            "scope": "all_queues" if args.all_queues else "active_queue",
            "head_sha": local_head,
            "reply_ids": reply_ids,
            ...
            if review:
                if str(review.get("state", "")).upper() == "DISMISSED":
                    ...
                    result = watcher_result(
                        ...
                        {
                            "result": (
                                "review_comments" if comments else "review_no_comments"
                            ),
                            "review_id": review["id"],
                            "review_url": review["html_url"],
    ...
    emit(
        {
            ...
            "state": str(path),
            "pr": state["pr"],
            "active_queue": state.get("active_queue"),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    preflight = subparsers.add_parser(
        "preflight", help="verify and check out a PR, then fetch the scoped queue"
    )
    ...
    publish.add_argument("--all-queues", action="store_true")
